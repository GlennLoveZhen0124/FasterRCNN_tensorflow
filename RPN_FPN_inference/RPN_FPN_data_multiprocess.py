...
class Data(object):
	...
	def read_image(self, start, end, files, data):
		for i in tqdm(range(start,end)):
			image_path = files[i]
			img = cv2.irmead(image_path)
			if img.shape != (640,640,3):
				img = cv2.resize(img, (640,640))
			data[i] = img
	
	def collect_data(self, workers = 4):
		files = os.listdir(self.dataset_root)
		files = [f for f in files if '.xml' not in f]
		num = len(files)
		files = [os.path.join(self.dataset_root, f) for f in files]
		...
		processes = []
		for i in range(workers):
			p = Process(target = self.read_image, args = (starts[i], ends[i], files, data))
			p.start()
			processes.append(p)
		for p in processes:
			p.join()
